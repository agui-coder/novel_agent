...
| 检查面 | 结果 | 证据 |
| --- | --- | --- |
| Dify 运行库提示词扫描 | 0 个硬编码、英文规约、乱码问题 | `.runtime/prompt_live_scan_schema_adaptive_after_inventory.json` |
| 可复跑补丁脚本扫描 | 0 个硬编码、英文规约、乱码问题 | `.runtime/prompt_script_scan_schema_adaptive_after_inventory.json` |
| 本地导出快照扫描 | 0 个硬编码、英文规约、乱码问题 | `.runtime/prompt_export_scan_schema_adaptive_after_inventory.json` |
...
| 世界模型agent | 初始化后阶段的世界观读取、解释、局部修订、在线考据与约束生命周期整理；读取 `summary.md` 时先识别旧段落约束格式或后端归档格式，再映射到统一蒸馏槽位。 | `scripts/patch_world_model_agent.py` | 已开启 | 只在明确写入时更新世界观、状态卡或相关约束文件 |
...
- `summary.md` 的当前生产入口是后端摘要归档管线；Dify `读书存档agent` 只保留历史兼容/审计用途。
- 世界模型 agent 不能依赖固定摘要标题；旧 `### 故事阶段` 格式和新 `## Batch Overview / Batch Index` 格式都必须先识别再蒸馏。
