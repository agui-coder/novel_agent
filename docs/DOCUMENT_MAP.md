...
- 当前 Dify Agent 工作流导出，包含活跃工作流、后初始化协作工作流和已退役历史兼容工作流。
- 是 Agent 配置证据，不是产品说明文档，也不是当前主链路归属的唯一真相。

当前工作流：

- `世界模型agent.yml`：保留，但初始化/重建职责已退役，转为后初始化协作。
- `文风学习agent.yml`：保留，但初始化/重建职责已退役，转为后初始化协作。
- `灵感大纲agent.yml`：活跃。
- `续写agent.yml`：活跃。
- `审核agent.yml`：活跃。
- `读书存档agent.yml`：已退役，仅历史兼容；摘要主链路已迁移到后端 LangChain 管线。

阅读建议：

- 调试活跃 Dify 工作流或核对历史 DSL 时查看。
