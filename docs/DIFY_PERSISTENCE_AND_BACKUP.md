...
```
<dify-compose-dir>/volumes/db/data  →  /var/lib/postgresql/data (容器内)
...
```powershell
wsl -d Ubuntu -- bash -lc "cd $NOVEL_AGENT_DIFY_COMPOSE_DIR && docker compose restart api worker plugin_daemon"
```
...
2. **bind mount 原地**（`<dify-compose-dir>/volumes/db/data`）— 容器重建后数据仍在，`docker compose up -d` 即可
...
- 不要手动删除 `<dify-compose-dir>/volumes/db/`
- 恢复前先跑一次备份脚本留存当前状态
