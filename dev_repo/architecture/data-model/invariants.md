...
- Historical review chat messages without `target_draft_commit` must not be silently synced into `ProseDeliveryState`. A pass-like historical message may be adopted only through an explicit author action, which binds it to the current `draft_commit` and records `review_report.review_is_author_approval=true`; the final archive/canonization click remains a separate author decision.
