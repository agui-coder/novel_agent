...
- `DraftSandbox`: `draft/sandbox` branch plus changed files used by review. It is branch-scoped: the review state includes a source plot branch and source commit so a draft created under one plot line cannot be reviewed or confirmed into another plot line by current-checkout accident.
- `ProseDeliveryState`: ignored derived control-plane state at `.loregit/prose_delivery_state.json` for the current `chapter_draft.md` delivery transaction. It binds `BookWorkspace`, `draft/sandbox`, source plot branch metadata, and `draft_commit`; stores parsed chapter spans, structured review findings plus the `source_draft_commit` they reviewed, whether a pass report was explicitly adopted by the author, per-finding rewrite requests, manual-edit save state, archive state, and post-confirm handoff state; and must not become source prose, hidden canon, or a substitute for the final author archive click.
...
- `ConversationMessageLog`: JSONL messages per conversation under `dev_repo/conversations/<agent>/<book_id>__<conversation_id>.jsonl`. Review-agent messages may carry optional `target_draft_commit` so the frontend can reject syncing an old review onto a newer prose draft.
...
- One `BookWorkspace` owns one `BookMetadata`, many `ChapterFile`, many `KnowledgeFile`, zero or one latest `ImportReport`, zero or one active `ProseDeliveryState`, one `GitRepositoryState`, and optionally one `PendingDeleteMarker`.
...
- One active `ProseDeliveryState` derives from one `DraftSandbox` and the current `chapter_draft.md` `KnowledgeFile`. It records the current prose review/archive transaction for a draft package, including chapter spans, review findings, the reviewed draft commit, manual edits, rewrite requests, archive state, and post-confirm handoff state. It is invalidated when the bound `draft_commit` changes, must reject review reports whose `source_draft_commit` does not match the current draft, and it must not outrank Git, `chapter_draft.md`, or explicit author approval.
- One `ProseDeliveryState` may record structured review findings that are also distilled into `error_archive.md` as reusable evidence. `error_archive.md` remains long-term memory; the current pass/problem/archive eligibility state belongs to `ProseDeliveryState`.
- One `RollingProductionRun` may hand a materialized continuation draft to one active `ProseDeliveryState`. The rolling layer may observe that handoff state, but it may not edit prose, approve the draft, or archive chapters without the prose delivery and draft confirm flow.
...
- One `DraftSandbox` is bound to one source plot branch in `GitRepositoryState`. Its `base_branch` and `base_commit` define the review baseline for changed files, diff preview, confirm, and rollback. The branch name may remain `draft/sandbox` for compatibility, but the semantic identity includes the bound source plot branch.
...
- Post-confirm archive maintenance is staged from accepted chapter canon to derived knowledge refresh: accepted `chapter_draft.md` sections become `ChapterFile` artifacts; the backend summary archive pipeline refreshes `summary.md`; the backend status projection refreshes `status_card.md` from that fresh summary. Optional world-route updates to `world_model.md` or `domain_rules.md` are reserved for durable world rules, identities, timeline/loop state, cosmology, contradiction repairs, or other long-lived constraints. The review route remains limited to `error_archive.md`.
