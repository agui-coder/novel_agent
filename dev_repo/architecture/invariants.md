...
- New backend world/status initialization must be schema-locked: model extraction returns structured world facts or an equivalent validated intermediate representation; `world_model.md` and `status_card.md` are rendered/projection outputs after lifecycle, scope, evidence, confidence, downstream-impact, and quality checks pass.
- `status_card.md` must not pass merely because required labels exist. Current-state projection must have evidence anchors and a bounded `???` ratio, or it must fail closed with an explicit error.
...
- Rolling chapter production must repair malformed pending chapter cards through the outline route when `repair_outline_cards` is reported; local orchestration may package diagnostics but must not silently rewrite `chapter_outline.md` or replace a repairable card with a new number.
- Rolling workbench outline handoff controls are promptless direct jobs: `repair_outline_cards` exposes "修复章节卡", `replenish_outline` exposes "生成下一批章节卡", and both must refresh rolling state before enabling continuation.
