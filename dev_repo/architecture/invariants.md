...
- Rolling chapter production must treat missing structured chapter-card fields as advisory diagnostics when a chapter number and readable outline text exist; those diagnostics must not trigger a repair scheduler action by themselves.
- Rolling workbench outline handoff controls are promptless direct jobs: `replenish_outline` exposes "生成下一批章节卡" and must refresh rolling state before enabling continuation.
