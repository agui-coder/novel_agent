...
| --- | --- | --- | --- | --- | --- | --- |
| `frontend-workbench` | Main author IDE: file view, chat, review, Git panels, streaming traces, FSM, promptless continuation controls, and the dedicated prose delivery workbench. | `frontend/src/App.tsx`, `frontend/src/store/index.ts`, `frontend/src/types/store.d.ts`, `frontend/src/components/*`, `frontend/src/api/*`, `frontend/src/lib/*` | Browser UI at `http://127.0.0.1:5173`; API clients in `frontend/src/api/*`; continuation workbench action surface; prose delivery workbench surface | `flask-backend-api`, `session-runtime`, `book-storage-git`, `dify-bridge` through Flask, `rolling-production-orchestrator` through Flask | Never writes storage directly; review state must be backed by draft metadata or `ProseDeliveryState`; user-visible state must survive hydration where persisted; promptless continuation controls must not become hidden chat prompts; `chapter_draft.md` prose delivery must not be inferred from filename-only UI heuristics. | confirmed |
...
| `book-storage-git` | Per-book filesystem layout, metadata, nested Git repository, branch/diff/review persistence, and ignored prose delivery control state. | `novel_git_server/utils/book_storage.py`, `novel_git_server/utils/git_utils.py`, `novel_git_server/agents/archive.py`, `novel_git_server/agents/git_console.py`, `novel_git_server/agents/checkout.py`, `novel_git_server/agents/history.py`, future `novel_git_server/utils/prose_delivery_state.py` | Storage APIs, Git console APIs, markdown read/write APIs, planned prose delivery state helpers | Local filesystem, Git, Flask helpers | Every book path must stay inside storage root; important writes must commit or return an explicit error; draft writes belong on `draft/sandbox`; each draft review state must be bound to its source plot branch before diff, review, confirm, or rollback; `.loregit/prose_delivery_state.json` is derived control-plane state and must not enter book Git history. | confirmed |
...
| `rolling-production-orchestrator` | Three-chapter rolling coordinator for long-form production: derive chapter cursor, select next batch, trigger continuation, request outline replenishment, expose workbench state, hand off finished drafts to prose delivery, and record evidence. | `novel_git_server/pipelines/rolling_chapter.py`, `scripts/rolling_chapter_production.py`, `docs/ROLLING_CHAPTER_WORKFLOW_CASE.md`, `.runtime/rolling_chapter_*.json` | CLI/dry-run planner, live probe scripts, case-study evidence, `/api/rolling/state`, `/api/rolling/run_stream` | `book-storage-git`, `dify-bridge`, `dify-workflows-runtime`, `dev-repo-runtime` | Orchestrator must not write prose; chapter_draft.md remains owned by continuation workflow; depleted chapter cards must be replenished by outline workflow; human review gates stop the loop; workbench state is a transient projection, not hidden source truth; once a draft package exists, current review/archive UI state belongs to `ProseDeliveryState`. | confirmed |
...
### Prose Delivery Workbench

Once the continuation route materializes `chapter_draft.md`, the current prose
review/archive state belongs to a dedicated prose delivery layer instead of the
generic diff review workspace. The backend persists this state as ignored
per-book metadata at `.loregit/prose_delivery_state.json`, represented by the
`ProseDeliveryState` data entity. The state binds `book_id`, `draft_branch`,
`base_branch`, `base_commit`, and `draft_commit`, so a review result cannot be
reused after the draft package changes or after the author switches plot lines.

`chapter_draft.md` is one draft package that may contain several chapter spans.
The prose delivery workbench may show those spans as separate review cards, but
it must remember that the source file is still one draft surface. A structured
review report may mark chapter spans or individual findings as pass/problem,
but review pass is not author approval. The author may manually edit and save
the draft in the delivery UI; that save creates a new draft commit and marks
older review reports stale. A rewrite request is always attached to a specific
review finding and hands the prior draft plus review advice back to the
continuation Agent, which remains the only AI writer for `chapter_draft.md`.

`error_archive.md` remains the long-term reusable evidence file for review
lessons. It must not become the live transaction state for "this draft is
passed", "this chapter has a blocking issue", or "archive is safe". Confirm,
rollback, branch mismatch, manual edit, rewrite, and post-confirm handoff
decisions must read the current `ProseDeliveryState` and draft metadata instead
of inferring status from chat bubbles or file names.
...
Draft review is branch-scoped. The compatibility branch name remains `draft/sandbox`, but the review state must remember which plot branch produced it. Diff, review, confirm, and rollback use that source branch as the baseline. A user checking out another plot branch must not make `draft/sandbox` appear as a draft for that branch, and confirm must not merge across plot lines by accident.
