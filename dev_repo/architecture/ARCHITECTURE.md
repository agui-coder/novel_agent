...
Malformed pending cards are also handled as an outline-layer handoff. If the next pending card exists but is not executable, the orchestrator reports `repair_outline_cards` and packages the blocked card diagnostics for the outline Agent. The frontend must expose a promptless "修复章节卡" action for that state. If the deck is depleted, it exposes "生成下一批章节卡". Both actions route to the outline Agent and refresh rolling state afterward; neither action allows the rolling layer or continuation Agent to fabricate or repair source outline content.
