...
- summary.md 可能是旧段落约束格式，也可能是后端归档格式；读取单位是"一个 Batch Archive 段落"，不是固定章数。
...
- summary.md 的输入格式必须先识别再蒸馏，禁止假定固定标题。当前兼容两类格式：
  - 旧段落约束格式：每个 ## Batch Archive: CHxx-{yy} 下可能包含 ### 故事阶段、### 不可撤销事实、### 核心驱动进度、### 线索状态、### 伏笔台账、### 未兑现承诺、### 关系变化、### 本段约束增量。
  - 后端归档格式：每个 ## Batch Archive: CHxx-{yy} 下可能包含 ## Batch Overview、## Batch Index、## Irreversible Facts、## Open Loops And Promises、## Relationship And State Changes、## Downstream Constraints。
- 不要按逐章流水账理解 summary.md。Batch Index 只是章节索引，必须从最高章节号与最近约束判断当前阶段；Batch Overview 是背景概览，不能替代最新状态。
...
- 段落遍历策略：拿到 outline 后，先从最新 Batch Archive 确认当前阶段、最新压力、开放承诺和下游约束。然后按时间顺序从最旧未覆盖段落开始提纯。已在 world_model.md 中充分覆盖的早期段落（有 summary_alignment 台账确认）可跳过；未覆盖的中间段落不可跳过。预算不足时：输出"已覆盖段落范围 + 未覆盖段落范围 + 建议下一轮起点"。
- 读取 summary.md 段落时，先映射到统一蒸馏槽位，不要照搬标题名。统一槽位为：阶段/最近进度、不可撤销事实、题材驱动进度、线索/开放问题、承诺/伏笔、关系/状态变化、下游约束、证据缺口。
- 旧格式映射：故事阶段 -> 阶段/最近进度；不可撤销事实 -> 不可撤销事实；核心驱动进度 -> 题材驱动进度；线索状态/伏笔台账/未兑现承诺 -> 线索/开放问题与承诺/伏笔；关系变化 -> 关系/状态变化；本段约束增量 -> 下游约束。
- 后端归档格式映射：Batch Overview -> 阶段背景；Batch Index -> 最近进度索引；Irreversible Facts -> 不可撤销事实；Open Loops And Promises -> 线索/开放问题与承诺/伏笔；Relationship And State Changes -> 关系/状态变化；Downstream Constraints -> 下游约束。
- 题材驱动进度必须自适应：升级流关注境界/突破，智斗关注信息差/规则揭示，轮回关注循环/蝴蝶效应，言情关注感情阶段，种田关注建设进度，末世关注安全/威胁，竞技/职业流关注赛事阶段、对手压力、排名/晋级、队伍状态、商业资源和职业承诺。不同题材的驱动子结构不同，按实际存在内容提取。
- 伏笔或开放问题如含可见性维度（读者已知/角色未知/仅角色已知），蒸馏时保留；如果原文没有可见性，不要强行补造。
