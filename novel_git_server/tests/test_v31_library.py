...
class V31LibraryTests(unittest.TestCase):
    ...
    def test_book_runtime_conversation_files_are_ignored_by_nested_git(self):
        init_resp = self.client.post(
            "/books/init",
            json={"book_id": "runtime_ignore", "book_name": "runtime ignore"},
        )
        self.assertEqual(init_resp.status_code, 200)
        ...
        runtime_files = [
            repo_dir / "conversations" / "world_agent" / "runtime_ignore__conv.jsonl",
            repo_dir / ".conversations" / "outline_agent" / "runtime_ignore.index.json",
    ...
    def test_existing_book_gitignore_runtime_update_is_committed_without_other_changes(self):
        init_resp = self.client.post(
            "/books/init",
            json={"book_id": "runtime_migrate", "book_name": "runtime migrate"},
        )
        self.assertEqual(init_resp.status_code, 200)

        repo_dir = self.temp_dir / "runtime_migrate"
        old_gitignore = "*.tmp\n*.log\n__pycache__/\n.locks/\nconflicts/\n"
        (repo_dir / ".gitignore").write_text(old_gitignore, encoding="utf-8")
        subprocess.run(
            ["git", "add", "--", ".gitignore"],
            cwd=repo_dir,
            check=True,
        ...
        subprocess.run(
            ["git", "commit", "-m", "test: simulate legacy gitignore", "--", ".gitignore"],
            cwd=repo_dir,
            check=True,
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
        ...
        (repo_dir / "chapter_draft.md").write_text("# local user draft\n\n不要被补 .gitignore 顺手提交。\n", encoding="utf-8")
        ...
        self.assertEqual(latest_subject, "chore: update book gitignore")
        status = subprocess.run(
            ["git", "status", "--short", "--untracked-files=all"],
            cwd=repo_dir,
            check=True,
        ...
        self.assertEqual(status, "?? chapter_draft.md")
