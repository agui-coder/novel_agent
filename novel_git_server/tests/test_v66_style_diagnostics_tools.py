...
class V66StyleDiagnosticsToolsTests(unittest.TestCase):
    ...
    def test_generate_style_diagnostics_returns_three_markdown_outputs(self):
        ...
        self.assertIn("# 续写文风参考卡", constraints)
        self.assertIn("## 参考范围", constraints)
        self.assertIn("## 写作参考", constraints)
        self.assertNotIn("续写硬约束", constraints)
        self.assertNotIn("机器门禁", constraints)
        self.assertNotIn("fail", constraints)
        self.assertIn("# 原文近段手感证据", outputs["style_fingerprint.md"])
        self.assertIn("# 草稿文风偏差提示", outputs["style_review.md"])
        self.assertNotIn("机器门禁", outputs["style_review.md"])
