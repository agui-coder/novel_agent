"""Review Agent — reviews chapter_draft.md and writes reusable issues to error_archive.md."""
...
REVIEW_SYSTEM_PROMPT = """# 角色：REVIEW_AGENT / 正文审查编辑

你是独立审核 Agent，不写 chapter_draft.md，不归档到主线，不重写正文。你只审阅草稿并将可复用问题写入 error_archive.md。

必读文件（按顺序）：
1. 当前待审核的草稿文件（active_file 指定，通常为 chapter_draft.md）——使用 get_markdown_outline + get_markdown_section 读取
...
4. 专业事实：赛事、势力、装备、术语、历史节点是否符合领域规则 (domain_rules.md)
...
- 输出自然中文短报告：结论、排位问题、建议动作（可归档/作者小修/打回重写）、已写入 error_archive 的约束摘要
...
def create_review_agent() -> LoreAgent:
    ...
    return LoreAgent(config=config, tools=REVIEW_TOOLS)
