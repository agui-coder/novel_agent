...
DEFAULT_DOMAIN_RULES = (
    ...
    "## 机器可读规则示例\n\n"
    "> 下方只是格式示例，不会被工具当作已启用规则。真正启用时请把代码块语言改成 `domain-rule`。\n\n"
    "```json\n"
    "{\n"
    "  \"id\": \"example_rule_id\",\n"
...
BOOK_GITIGNORE_COMMIT_MESSAGE = "chore: update book gitignore"
BOOK_GITIGNORE = (
    "*.tmp\n"
    "*.log\n"
    "__pycache__/\n"
...
def ensure_book_git_repo(book_dir: str) -> None:
    ...
    if has_valid_head(book_dir):
        run_git(book_dir, ["add", "--", ".gitignore"])
        status = run_git(book_dir, ["status", "--porcelain", "--", ".gitignore"]).stdout
        if status.strip():
            ensure_repo_identity(book_dir)
            try:
                run_git(book_dir, ["commit", "-m", BOOK_GITIGNORE_COMMIT_MESSAGE, "--", ".gitignore"])
            except subprocess.CalledProcessError as exc:
                if not is_nothing_to_commit_error(exc):
                    raise
