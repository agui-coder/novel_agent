...
## 输出格式：
严格输出 JSON 对象，包含以下 7 个字段。每个字段的值为该批次在该维度的增量文档片段，无增量则为空字符串。
...
{{
  ...
  "downstream_interface": "对续写/审核/大纲/文风智能体的增量指令"
}}
...
3. 如果该维度无增量，输出空字符串 ""
4. 字段名保持上述 JSON 键名不变，字段值必须使用简体中文；专有名词可以保留原文
5. 直接输出 JSON，不要包裹在代码块中"""
