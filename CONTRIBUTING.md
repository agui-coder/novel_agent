...
`cargo tauri dev` 会自动编译 Rust、启动前端。改 Rust 代码需重启命令；只改 `desktop/src/` 则 Vite 热更新。

## 项目结构

开发只发生在 `desktop/` 下。`novel_git_server/`、`frontend/`、`dify_workflows/` 是旧 Python 项目，已废弃。
...
完整架构说明见 `CLAUDE.md`（AI 编码规范），技术历史见 `DEVELOPMENT.md`（注意该项目文档是 Python 时代的，只作参考）。
