...
> **适用范围**：本目录下的 Rust 后端 + React 前端 + Tauri 胶水层。
> **旧项目**：`novel_git_server/`（Python Flask）、`frontend/`（React）、`dify_workflows/` 是 Python 时代遗留，不再维护，**不要在上面改代码**。技术债见 `技术债.md`。
...
| 目录 | 状态 | 说明 |
|------|------|------|
| `src-tauri/` | **活跃** | Rust 后端，单体 crate，Tauri v2 |
| `src/` | **活跃** | React 19 前端，Zustand 状态管理 |
| `src-tauri/prompts/` | **活跃** | AI Agent 的 system prompt，`include_str!` 嵌入 |
| `.claude/skills/` | **活跃** | 开发工作流 Skill（Code review、收尾清单等） |
| `novel_git_server/` | 废弃 | Python Flask 后端，已被 Rust 替代 |
| `frontend/` | 废弃 | 旧前端，已被 `src/` 替代 |
| `dify_workflows/` | 废弃 | Dify DSL，已被 Rust engine 替代 |
| `storage/` | 运行时 | 书籍数据（Markdown + .git），勿提交到 git |

**原则**：新功能只在 本目录 下实现。旧代码可参考逻辑，不照搬实现。
...
在 `src-tauri/prompts/` 下新建 `.md` 文件后：
1. 在 `src-tauri/src/engine/prompts.rs` 中加 `pub const XXX_PROMPT: &str = include_str!("../../prompts/xxx.md");`
2. 在 `src-tauri/src/engine/agent.rs` 的 `AgentType` 枚举中加变体（如需新增 Agent 类型）
...
当前 `src-tauri/tests/unit_tests.rs` 中重复实现了生产代码——**禁止继续此模式**。测试必须 `use` 被测试的生产代码，不要复制粘贴。
...
### 8.2 Agent Prompt（`src-tauri/prompts/`）
...
- [ ] 文件位置正确（Skill → `.claude/skills/`，Agent Prompt → `src-tauri/prompts/`）
...
| 在 `lib.rs` 的 `generate_handler!` 中重复注册命令 | 已知技术债（见 `技术债.md` §1.2） |
| 在 store 中直接调用 `invoke()` | 违反分层 |
| 新增依赖全局 `OnceLock` 的隐式状态 | 改用参数注入（见 `技术债.md` §2.5） |
...
每次在 本目录 下完成代码修改后，执行以下步骤。详见 Skill `post-change-checklist`。

1. **代码清理**：删除无调用方的函数/import/模块、Python 残留
2. **链路走查**：通读 diff，核对 IPC/事件/工具调用链路
3. **Rust 审查**：检查 unwrap、unsafe、并发、错误处理
4. **`cargo check --manifest-path src-tauri/Cargo.toml`**（0 error）
5. **`cargo test --manifest-path src-tauri/Cargo.toml`**（全绿）
6. **`npm run build`**（前端 build 通过）
