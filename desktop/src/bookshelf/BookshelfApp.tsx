...
export function BookshelfApp() {
    ...
    const refreshBooks = useCallback(async (options?: { updatePageState?: boolean }): Promise<BookListItem[]> => {
        ...
        try {
            const list = await fetchBooksList();
            setBooks(list);
            if (updatePageState) {
                setPageState(list.length === 0 ? 'empty' : 'bookshelf');
            }
            return list;
        } catch {
            if (updatePageState) {
    ...
    const handleCreateProject = async () => {
        ...
        try {
            ...
            const result = await invoke<{ status: string; book_id: string; book_name: string; created: boolean }>('init_book', { bookName: name });
            setToast({ text: `项目「${name}」已创建`, ts: Date.now() });
            await refreshBooks();
            handleEnterBook(result.book_id);
        } catch (e: any) {
            setToast({ text: `创建失败: ${e?.message || String(e)}`, ts: Date.now() });
        } finally {
            setCreating(false);
            setNewProjectName('');
        }
            ...
            <div className="relative z-10 flex w-full max-w-4xl flex-col items-center gap-8 px-8">
                ...
                {pageState === 'empty' && (
                    <div className="flex flex-col items-center gap-8">
                        {/* New Project */}
                        <div className="flex flex-col items-center gap-4 rounded-[20px] border border-[rgba(115,134,255,0.12)] bg-[rgba(115,134,255,0.03)] px-12 py-10">
                            <div className="text-center">
                                <div className="text-lg font-semibold text-[var(--color-dark-text-main)]">创建新项目</div>
                                <div className="mt-2 max-w-sm text-sm text-[var(--color-dark-text-muted)]">
                                    输入书名，创建一个全新的创作项目。后续可以导入章节或从零开始。
                                </div>
                            </div>
                            <div className="flex gap-3">
                                <input
                                    ...
                                    placeholder="输入书名..."
                                    disabled={creating}
                                    className="rounded-[10px] border border-[rgba(255,255,255,0.12)] bg-[rgba(255,255,255,0.04)] px-4 py-2.5 text-sm text-[var(--color-dark-text-main)] placeholder:text-[var(--color-dark-text-faint)] outline-none transition-colors focus:border-[rgba(115,134,255,0.4)] disabled:opacity-50 w-48"
                                    autoFocus
                                />
                                <button
                                    type="button"
                                    ...
                                    className="rounded-[10px] border border-[rgba(115,134,255,0.32)] bg-[rgba(115,134,255,0.12)] px-5 py-2.5 text-sm font-semibold text-[var(--color-dark-text-main)] transition-colors hover:bg-[rgba(115,134,255,0.22)] disabled:cursor-not-allowed disabled:opacity-50"
                                >
                                    {creating ? '创建中...' : '创建'}
                                </button>
                            </div>
                        ...
                        {/* Or import */}
                        <div className="flex flex-col items-center gap-4 rounded-[20px] border border-[rgba(255,255,255,0.06)] bg-[rgba(255,255,255,0.02)] px-12 py-10">
                            <div className="text-center">
                                <div className="text-sm font-medium text-[var(--color-dark-text-muted)]">或者从已有小说导入</div>
                            </div>
                            <button
                                type="button"
                                onClick={handleOpenImport}
                                disabled={summaryBusy}
                                className="rounded-[10px] border border-[rgba(255,255,255,0.12)] bg-[rgba(255,255,255,0.04)] px-5 py-2.5 text-sm text-[var(--color-dark-text-muted)] transition-colors hover:border-[rgba(115,134,255,0.24)] hover:text-[var(--color-dark-text-main)] disabled:cursor-not-allowed disabled:opacity-50"
                            >
                                导入番茄小说
                            </button>
                        </div>
                ...
                {(pageState === 'bookshelf' || pageState === 'importing') && (
                    <>
                        ...
                        <div className="flex items-center gap-3 rounded-[12px] border border-[rgba(115,134,255,0.08)] bg-[rgba(115,134,255,0.02)] px-4 py-3">
                            <span className="text-xs font-medium text-[var(--color-dark-text-muted)] whitespace-nowrap">新建项目</span>
                            <input
                                type="text"
                                value={newProjectName}
                                onChange={(e) => setNewProjectName(e.target.value)}
                                ...
                                placeholder="书名..."
                                disabled={creating}
                                className="rounded-[8px] border border-[rgba(255,255,255,0.10)] bg-[rgba(255,255,255,0.04)] px-3 py-1.5 text-xs text-[var(--color-dark-text-main)] placeholder:text-[var(--color-dark-text-faint)] outline-none w-36 disabled:opacity-50"
                            />
                            <button
                                type="button"
                                onClick={handleCreateProject}
                                disabled={creating || !newProjectName.trim()}
                                className="rounded-[8px] border border-[rgba(115,134,255,0.24)] bg-[rgba(115,134,255,0.08)] px-3 py-1.5 text-xs font-semibold text-[var(--color-dark-text-main)] transition-colors hover:bg-[rgba(115,134,255,0.16)] disabled:cursor-not-allowed disabled:opacity-50"
                            >
                                {creating ? '...' : '创建'}
                            </button>
                        </div>
