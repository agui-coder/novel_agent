...
export default function App() {
    ...
    // Ref for late-bound deps provided by useDraftReview/useGitWorkbench/useAgentSession
    const chatStreamLateRef = useRef<{ loadReviewTargetMainline: (file: string) => Promise<void>; loadGitWorkbench: () => Promise<void>; loadConversationContext: (agentOverride?: AgentKey, conversationIdOverride?: string | null) => Promise<void>; hasPendingDraftDecision: boolean }>({
        loadReviewTargetMainline: async () => {},
        loadGitWorkbench: async () => {},
        loadConversationContext: async () => {},
        hasPendingDraftDecision: false,
    });

    const {
        suppressedBackendErrors, setSuppressedBackendErrors,
        suppressedDebugOpen, setSuppressedDebugOpen,
        ...
        beginRewriteFromMessage, handleCancelRewrite, handleInlineRewriteSubmit,
        streamAbortControllerRef, streamTaskIdRef, streamStableUpstreamConversationIdRef,
    } = useChatStream({
        store,
        repoIntegrity,
        setRepoIntegrity,
        loadMainline,
        setRewriteUserMessageId,
        setCommandInput,
        rewriteUserMessageId,
        commandInput,
        lateRef: chatStreamLateRef,
    });
    ...
    // Update late-bound deps
    draftReviewLateRef.current = { hasPendingDraftDecision, loadReviewTargetMainline };
    chatStreamLateRef.current = { ...chatStreamLateRef.current, hasPendingDraftDecision, loadReviewTargetMainline };
    ...
    chatStreamLateRef.current.loadGitWorkbench = loadGitWorkbench;
    ...
    chatStreamLateRef.current.loadConversationContext = loadConversationContext;
