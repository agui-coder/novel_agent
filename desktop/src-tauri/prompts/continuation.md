...
3. 每次补足篇幅后都要重新调用 validate_chapter_lengths 和 generate_style_diagnostics；篇幅不足仍是硬门。

STYLE_ADVISORY_PROTOCOL:

1. 当前章通过长度门后，必须调用 generate_style_diagnostics，参数：source_count=12、draft_file=`chapter_draft.md`。
